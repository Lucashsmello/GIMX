...
static void handleReport(s_network_packet_in_report * report, int adapter) {
	...
	for (i = 0; i < report->nbAxes; ++i) {
		...
		if (offset < AXIS_MAX) {
			...
			gwarn("%s: bad axis index: %s %hu\n", __func__, (report->axes[i].index & 0x80) ? "abs" : "rel",
					report->axes[i].index & 0x7f);
		}
	}
  ...
  case E_NETWORK_PACKET_IN_REPORT:
    {
		...
		if ((unsigned int) nread != sizeof(*report) + report->nbAxes * sizeof(*report->axes)) {
			gwarn("%s: wrong packet size: %u %zu\n", __func__, nread,
					sizeof(*report) + report->nbAxes * sizeof(*report->axes));
			return 0;
		}
		handleReport(report,adapter);
    }
    break;
    ...
    if (gimx_params.force_updates || adapter->send_command)
    {
      if(adapter->atype == E_ADAPTER_TYPE_REMOTE_GIMX)
      {
        if(adapter->remote.fd >= 0)
        {
	      ...
	      //printf(">%d %d\n",adapter->axis[i],report->axes[report->nbAxes].value);
              ++report->nbAxes;
            }
          }
